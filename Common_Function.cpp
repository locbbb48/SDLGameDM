...
SDL_Surface* SDLCommonFunc::LoadImage(std::string file_path) {
	...
	SDL_Surface* optimate_image = NULL;
	load_image = IMG_Load(file_path.c_str());

	if (load_image != NULL) {
		optimate_image = SDL_DisplayFormat(load_image);
		SDL_FreeSurface(load_image);
	}

	if (optimate_image != NULL) {
		UINT32 color_key = SDL_MapRGB(optimate_image->format, 0, 255, 255);
		SDL_SetColorKey(optimate_image, SDL_SRCCOLORKEY, color_key);
	}

	return optimate_image;
}
